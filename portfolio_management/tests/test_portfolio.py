...
def test_portfolio_implementation(portfolio, return_data):
    ...
    for date in dates:
        ...
        aapl_weight = np.random.randint(low=-10, high=11) / 10

        msft_sign = np.random.choice([-1, 1])
        msft_weight = msft_sign * (1 - abs(aapl_weight))
        ...
        portfolio_value_records.append(
            {'date': date, 'portfolio_value': latest_portfolio_value})
    ...
    assert portfolio.initial_cash == portfolio.nav_history[
        0]['nav'], 'Initial cash and initial NAV must match!'
    assert nav['cash'].min() >= 0, 'Cash can never be negative!'
    assert (nav['nav'] == nav['portfolio_value']).all(
    ), 'Portfolio value calculations are incorrect!'
