...
def test_signal_generation(strategy, return_data):
    ...
    assert strategy.signal_needed(dates[0]), 'On the first day we require a signal!'
    ...
    assert (signals['weight_sign'] == signals['return_1w_sign']).all(), 'Signal generation does not line up with momentum strategy logic!'
    
    dates = signals.index.get_level_values('date').unique()
    date_gaps = pd.Series(dates, name='days_since_last_signal').diff().dt.days.dropna()
    
    assert (date_gaps >= strategy.lookback_days).all(), 'Signals generated too early!'
